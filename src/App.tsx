...
import ProductCategoryTrends from "./pages/ProductCategoryTrends";
import ProductCategoryProgress from "./pages/ProductCategoryProgress";
import MaterialCategoryMaster from "./pages/MaterialCategoryMaster";
...
function ProtectedRoute({ children }: { children: ReactNode }) {
  ...
  if (user.mfa_temporarily_exempt) return <>{children}</>;
  if (!mfaStatus?.isEnrolled)
    return <Navigate to="/mfa/setup" replace state={{ from }} />;
  if (!mfaStatus.isVerified)
    return <Navigate to="/mfa/verify" replace state={{ from }} />;
  return <>{children}</>;
}
...
function MfaRoute({ children }: { children: ReactNode }) {
  ...
  const homePath =
    redirectPath ||
    (isMobile || !user?.can_view_dashboard ? "/deals/new" : "/dashboard");
  ...
  if (user.must_change_password)
    return <Navigate to="/initial-password" replace />;
  if (user.mfa_temporarily_exempt) return <Navigate to={homePath} replace />;
...
function UserManagementRoute({ children }: { children: ReactNode }) {
  ...
  if (!user?.can_manage_users) {
    return (
      <Navigate
        to={user?.can_view_dashboard ? "/dashboard" : "/deals/new"}
        replace
      />
    );
  }
...
function InitialPasswordRoute() {
  ...
  if (!user.must_change_password)
    return (
      <Navigate
        to={user.mfa_temporarily_exempt ? "/login" : "/mfa/setup"}
        replace
      />
    );
...
function AuthRedirect({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (user.mfa_temporarily_exempt) {
    ...
    if (!mfaStatus?.isEnrolled) {
      navigate("/mfa/setup", {
        state: { from: redirectPath || homePath },
        replace: true,
      });
      return;
    }
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route
            ...
            element={
              <ProtectedRoute>
                <ClinicAssetsDashboard />
              </ProtectedRoute>
            }
          />
          <Route
            path="/clinic-sales-trend"
            element={
              <ProtectedRoute>
                <ClinicSalesTrend />
              </ProtectedRoute>
          ...
          <Route
            ...
            element={
              <ProtectedRoute>
                <ProductCategoryTrends />
              </ProtectedRoute>
            }
          />
          <Route
            path="/product-category-progress"
            element={
              <ProtectedRoute>
                <ProductCategoryProgress />
              </ProtectedRoute>
            }
          />
          <Route
