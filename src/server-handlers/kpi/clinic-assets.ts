...
async function fetchCareDeals(customerCodes: string[], endExclusiveDate: string) {
  const chunks = Array.from({ length: Math.ceil(customerCodes.length / 200) }, (_, index) => (
    customerCodes.slice(index * 200, (index + 1) * 200)
  ));
  const results = await Promise.all(chunks.map(async (codes) => {
    const { data, error } = await supabaseAdmin
      ...
      .lt('deal_date', endExclusiveDate)
      .order('deal_date', { ascending: false })
      .order('created_at', { ascending: false });
    if (error) throw error;
    return (data ?? []) as CareDealRow[];
  }));
  return results.flat().sort((left, right) => (
    String(right.deal_date ?? right.created_at ?? '').localeCompare(String(left.deal_date ?? left.created_at ?? ''))
  ));
}

async function fetchCareDealStates(dealIds: string[], targetMonth: string) {
  const chunks = Array.from({ length: Math.ceil(dealIds.length / 200) }, (_, index) => (
    dealIds.slice(index * 200, (index + 1) * 200)
  ));
  const results = await Promise.all(chunks.map(async (ids) => {
    ...
    if (error) throw error;
    return (data ?? []) as DealBoardStateRow[];
  }));
  return results.flat().sort((left, right) => String(right.month_start).localeCompare(String(left.month_start)));
}
...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const careDeals = assetCustomerCodes.length > 0
      ? await fetchCareDeals(assetCustomerCodes, toDateString(displayEndExclusive))
      : [];
    const careDealIds = careDeals.map((row) => row.id).filter(Boolean);
    const careDealStateRows = careDealIds.length > 0
      ? await fetchCareDealStates(careDealIds, targetMonth)
      : [];

    const latestStateByDealId = new Map<string, DealBoardStateRow>();
    careDealStateRows.forEach((row) => {
    ...
    markDebug('build_asset_rows', {
      careDealRows: careDeals.length,
      careDealStateRows: careDealStateRows.length,
