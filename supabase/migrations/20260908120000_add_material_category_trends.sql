...
create materialized view if not exists public.material_category_department_daily_actuals as
select
  h.department_id,
  m.material_category as category_name,
...
join public.material_category_masters m
...
where h.department_id in (1, 2)
  ...
  and m.is_active
  and m.material_category <> '未分類'
group by h.department_id, m.material_category, h."納品日"::date,
  nullif(btrim(h."得意先コード"::text), '');

create unique index if not exists material_category_department_daily_actuals_idx
...
language sql security definer set search_path = public as $$
  select daily.department_id, '材料売上｜' || daily.category_name, 1000,
...
language sql security definer set search_path = public as $$
  select daily.department_id, '材料売上｜' || daily.category_name,
    extract(year from (daily.sales_date - interval '3 months'))::integer,
...
begin
  refresh materialized view public.material_category_department_daily_actuals;
end;
$$;
