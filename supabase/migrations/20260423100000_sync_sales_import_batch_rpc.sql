...
create or replace function public.normalize_sales_import_date(p_value text)
returns date
language sql
immutable
as $$
  select case
    ...
    when replace(replace(btrim(p_value), '.', '/'), '-', '/') ~ '^\d{4}/\d{1,2}/\d{1,2}$'
      then to_date(replace(replace(btrim(p_value), '.', '/'), '-', '/'), 'YYYY/MM/DD')
    else null
  end;
$$;

create or replace function public.parse_sales_import_amount(p_value text)
returns bigint
language sql
immutable
as $$
  select coalesce(
    round(nullif(regexp_replace(coalesce(p_value, ''), '[,\s]', '', 'g'), '')::numeric),
    0
  )::bigint;
$$;
...
grant execute on function public.sync_sales_import_batch(bigint, text)
